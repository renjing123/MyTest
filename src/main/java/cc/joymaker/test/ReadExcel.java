...
public class ReadExcel {
	...
	private static void read() throws Exception {
		InputStream is = new FileInputStream("/Users/mac/Desktop/5JM9dg.csv");
		...
		for (int numSheet = 0; numSheet < hssfWorkbook.getNumberOfSheets(); numSheet++) {
			...
			for (int rowNum = 1; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
				...
				XSSFCell yxsmc = hssfRow.getCell(0);
				String phone = getValue(yxsmc);
				if(phone == null) {
					phone = getValue(hssfSheet.getRow(rowNum - 1).getCell(0));
				}
				String special_no = getValue(hssfRow.getCell(1));
				String material_id = getValue(hssfRow.getCell(2));
				String material_name = getValue(hssfRow.getCell(3));
				String point = getValue(hssfRow.getCell(4));

				Map<String, String> m = new HashMap<String, String>();
				m.put("phone", phone);
				m.put("special_no", special_no);
				m.put("material_id", material_id);
				m.put("material_name", material_name);
				m.put("point", point);
	...
	private static String getValue(XSSFCell hssfCell) {
	 ...
	 private static void testWrite(List<Map<String, String>> list, int cloumnCount) {
		 // 第一步，创建一个webbook，对应一个Excel文件  
	        ...
	        cell.setCellValue("special_no");  
	        cell.setCellStyle(style);  
	        cell = row.createCell((short) 2);  
	        cell.setCellValue("material_id");  
	        cell.setCellStyle(style);  
	        ...
	        for (int j = 0; j < list.size(); j++) {  
                ...
                String special_no = dataMap.get("special_no").toString();  
                String material_id = dataMap.get("material_id").toString();  
                String material_name = dataMap.get("material_name").toString();  
                String point = dataMap.get("point").toString();  
                for (int k = 0; k <= cloumnCount; k++) {  
                // 在一行内循环  
                Cell first = row.createCell(0);  
                first.setCellValue(phone);  
          
                Cell second = row.createCell(1);  
                second.setCellValue(special_no);  
          
                Cell third = row.createCell(2);  
                third.setCellValue(material_id);
                
                Cell four = row.createCell(3);  
                third.setCellValue(material_name); 
                
                Cell five = row.createCell(4);  
                third.setCellValue(point); 
                }  
            }
	        ...
	        try  
	        {  
	            FileOutputStream fout = new FileOutputStream("/Users/mac/Desktop/111.xlsx");  
	            wb.write(fout);  
	            fout.close();  
	        }  
	        catch (Exception e)  
